...
Процент одинаковый, но бизнес-эффект разный. Поэтому в модели есть объемный и доверительный множители.
...
## Зоны здоровья
1. `healthIndex >= 85` — Норма
2. `70..84` — Внимание
3. `< 70` — Критично

## Визуальные зоны на bubble matrix (главная `/`)
Чтобы зона на графике отражала долю проблемных обращений и была визуально пропорциональна риску:
1. Зеленая: `0–5%`
2. Желтая: `5–30%`
3. Красная: `30–100%`

Это именно правила визуального разделения по оси X на главной странице.
Они не заменяют расчет `healthIndex`, который считается отдельной формулой выше.

## Почему это работает
1. `%` компонент (problemRate + severityRate) отражает качество и структуру негатива.
2. `volumeWeight` усиливает риск для массовых продуктов.
3. `confidence` снижает переоценку на маленьких объемах.

Итог: одинаковый процент на разных объемах дает разный риск, что ближе к реальной бизнес-картине.

## Где это используется в коде
- `lib/product-situation-analytics.ts`
